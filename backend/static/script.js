document.addEventListener('DOMContentLoaded', function () {
    ...
    const setupDbBtn = document.getElementById('setup-db-btn'); // New button for setup
    ...
    function switchMode(mode) {
        ...
        if (modeDatabaseBtn) modeDatabaseBtn.classList.toggle('active', mode === 'database');
        if (modeSerpapiBtn) modeSerpapiBtn.classList.toggle('active', mode === 'serpapi');

        // Update title
        if (currentModeTitle) currentModeTitle.textContent = mode === 'database' ? 'Database Mode' : 'Web Search Mode';
    ...
    if (modeDatabaseBtn) modeDatabaseBtn.addEventListener('click', () => switchMode('database'));
    if (modeSerpapiBtn) modeSerpapiBtn.addEventListener('click', () => switchMode('serpapi'));
    ...
    function addMessage(text, type = 'assistant', downloadData = null) {
        ...
        avatar.className = 'avatar'; // Changed from message-avatar to match CSS
        avatar.innerHTML = type === 'user' ? '<i class="fa-solid fa-user"></i>' : '<i class="fa-solid fa-robot"></i>';

        const content = document.createElement('div');
        content.className = 'content'; // Changed from message-content to match CSS
    ...
    function formatResults(results, summary, data) {
        ...
        if (summary) {
            ...
            html += `<div class="result-summary" style="margin-bottom: 1rem; padding: 1rem; background: rgba(99, 102, 241, 0.1); border-radius: 0.5rem; border-left: 3px solid var(--primary); line-height: 1.6;">${formattedSummary}</div>`;
        }
        ...
        if (data && data.collections_searched && data.collections_searched.length > 0) {
            ...
            if (!summary || !summary.toLowerCase().includes('collection')) {
                html += `<div style="margin-bottom: 1rem; padding: 0.75rem; background: rgba(16, 185, 129, 0.1); border-radius: 0.5rem; font-size: 0.9rem; color: var(--text-muted);">
        ...
        orderedKeys.forEach(k => {
            ...
            html += `<th style="${headerStyle} padding: 0.5rem 0.75rem; border: 1px solid var(--glass-border); text-align: left; font-weight: 600; font-size: 0.85rem; white-space: nowrap; width: ${width}; min-width: ${width}; max-width: ${width};">${escapeHtml(k)}</th>`;
        });
        html += '</tr>';
        ...
        results.forEach((r, idx) => {
            ...
            orderedKeys.forEach(k => {
                ...
                if (cellContent == null || cellContent === '') {
                    cellContent = '<span style="color: var(--text-muted); font-style: italic;">N/A</span>';
                } else if (k === 'Link' && cellContent) {
                    // Handle Link column
                    const linkText = String(cellContent);
                    cellContent = `<a href="${escapeHtml(linkText)}" target="_blank" rel="noopener noreferrer" style="color: var(--secondary); text-decoration: underline; white-space: nowrap;">View</a>`;
                } else if (k === '_collection') {
                    // Highlight collection column - compact
                    cellStyle = 'background: rgba(99, 102, 241, 0.1); font-weight: 600; color: var(--primary); white-space: nowrap;';
                    ...
                    cellStyle = 'font-style: italic; color: var(--text-muted); line-height: 1.4; white-space: normal; word-wrap: break-word;';
                    ...
                    try {
                        ...
                        cellStyle = 'font-family: monospace; font-size: 0.8rem; white-space: normal; word-wrap: break-word;';
                    } catch (e) {
                        cellContent = escapeHtml(String(cellContent));
                    }
                } else {
                ...
                html += `<td ${titleAttr} style="${cellStyle} ${wrapStyle} padding: 0.5rem 0.75rem; border: 1px solid var(--glass-border); width: ${width}; min-width: ${width}; max-width: ${width}; vertical-align: top;">${cellContent}</td>`;
            });
            html += '</tr>';
        });
    ...
    // ========== SETUP DB ==========
    if (setupDbBtn) {
        setupDbBtn.addEventListener('click', async function () {
            showToast('Setting up sample database...', 'info');
            try {
    ...
    async function loadCollections() {
        ...
        try {
            ...
            if (collections.length === 0) {
                collectionsList.innerHTML = '<div style="padding: 10px; color: var(--text-muted); font-size: 0.8rem;">No collections found</div>';
            } else {
                collectionsList.innerHTML = collections.map(c =>
                    `<div class="collection-item" style="display: flex; justify-content: space-between; padding: 8px 12px; background: rgba(255,255,255,0.03); border-radius: 6px; margin-bottom: 6px; font-size: 0.9rem;">
                        <span style="color: var(--text-main);">${escapeHtml(c.name)}</span>
                        <span style="background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 10px; font-size: 0.75rem;">${c.count}</span>
                    </div>`
                ).join('');
            }
        } catch (error) {
            console.error('Error loading collections:', error);
            collectionsList.innerHTML = '<div style="padding: 10px; color: var(--error); font-size: 0.8rem;">Error loading collections</div>';
        }
    }
    ...
    function showToast(message, type = 'info') {
        ...
        // Simple toast styling if not in CSS
        if (!toast.classList.contains('styled')) {
