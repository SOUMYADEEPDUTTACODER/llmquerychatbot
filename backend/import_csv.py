...
import_csv.py
-------------
Utility script to import CSV files into MongoDB collections.
You can import a single file or an entire folder of CSVs.
"""
...
def infer_value_type(value: str):
    """Automatically infer value types (int, float, bool, None, str)."""
    if value is None:
        return None
    v = value.strip()
    if v == "":
...
def import_csv_directory(client: MongoClient, dir_path: str, drop_existing: bool = True) -> Dict[str, Any]:
    """Import all CSV files from a directory (one per collection)."""
